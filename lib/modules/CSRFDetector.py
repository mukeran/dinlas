...
class CSRFDetector:
    def __init__(self, results, reports, **kwargs):
        report = {'title': 'CSRF Detector',
                  'overview': "CSRF vulnerabilities is an attack that forces an end user to execute "
                              ...
                              "<br> This Detector can automatic detect CSRF Token and try to detect 'Referer' header check."
    ...
    def find_token(self, form, form2):
        """
    ...
    def report(self, url, method, data, req, req_no_ref):
        ...
        text_no_ref = ''.join([str(res.status_code) + '->' for res in req_no_ref.history]) + str(req_no_ref.status_code) + ' '
        text_no_ref += req_no_ref.text if len(req_no_ref.text) < 150 else ' data length: {}'.format(len(req_no_ref.text))
        entry = [url, method, urlencode(data), text, text_no_ref]

        self.csrf_report['entries'].append(entry)

    def craft_field(self, form, location):
    ...
    def exec(self):
        ...
        for form_ in self.results['requests'].values():
            ...
            for form_index, form in enumerate(forms):
                ...
                # Referer check
                data = self.craft_field(form, location)
                # if method == 'GET':
